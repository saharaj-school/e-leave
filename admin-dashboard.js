...
(async function() {
    try {
        ...
        // View personnel detail - UPDATED WITH ALL 11 LEAVE TYPES
        window.viewPersonnelDetail = async function(userId) {
            try {
                ...
                message += `📊 วันลาคงเหลือ (ทั้ง 11 ประเภท):\n\n`;
                message += `🏥 ลาป่วย: ${user.sickLeaveRemaining || 30} วัน\n`;
                message += `👶 ลาคลอดบุตร: ${user.maternityLeaveRemaining || 90} วัน\n`;
                message += `🤱 ลาช่วยภริยาคลอดบุตร: ${user.helpWifeLeaveRemaining || 15} วัน\n`;
                message += `📝 ลากิจส่วนตัว: ${user.personalLeaveRemaining || 45} วัน\n`;
                message += `🏖️ ลาพักผ่อน: ${user.vacationLeaveRemaining || 10} วัน\n`;
                message += `🙏 ลาอุปสมบท: ${user.ordinationLeaveRemaining || 120} วัน\n`;
                message += `📚 ลาศึกษา: ${user.studyLeaveRemaining || 365} วัน\n`;
                message += `🌏 ลาองค์การระหว่างประเทศ: ${user.internationalLeaveRemaining || 730} วัน\n`;
                message += `💪 ลาฟื้นฟูสมรรถภาพ: ${user.rehabLeaveRemaining || 180} วัน\n`;
                message += `✈️ ลาติดตามคู่สมรส: ${user.followSpouseLeaveRemaining || 365} วัน\n`;
                message += `🏛️ ลาปฏิบัติงานในหน่วยงานอื่น: ${user.workOtherLeaveRemaining || 365} วัน`;
        ...
        // Edit personnel - UPDATED WITH ALL 11 LEAVE TYPES
        ...
        window.exportReport = function() {
            alert('📥 กำลังสร้างไฟล์รายงาน...\n\nไฟล์รายงานจะถูกดาวน์โหลดในรูปแบบ Excel');
        };
